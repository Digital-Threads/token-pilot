    ...
    inputSchema: {
      ...
      properties: {
        ...
        scope: {
          ...
          description:
            "Output scope: full (default, all details), nav (names + lines only, 2-3x smaller), exports (public API only)",
        },
        max_tokens: {
          type: "number",
          description:
    ...
    inputSchema: {
      ...
      properties: {
        ...
        show: {
          ...
          description:
            "Display mode: full (all lines), head (first 50), tail (last 30), outline (head + methods + tail). Default: auto (full ≤300 lines, outline >300)",
        },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        symbols: {
          ...
          items: { type: "string" },
          description:
            'Array of symbol names (max 10), e.g. ["UserService.create", "UserService.update", "UserService.delete"]',
        },
        context_before: {
          type: "number",
    ...
    inputSchema: {
      ...
      properties: {
        ...
        heading: {
          type: "string",
          description:
            'Section heading (Markdown), top-level key (YAML/JSON), or row range "rows:1-50" (CSV). Case-insensitive.',
        },
      },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        symbols: {
          ...
          items: { type: "string" },
          description:
            'Array of symbol names for batch edit context (max 10). Alternative to single "symbol" — returns all symbols in one call.',
        },
        line: {
          type: "number",
        ...
        include_callers: {
          type: "boolean",
          description:
            "Show top callers of this symbol (saves a separate find_usages call)",
        },
        include_tests: {
          type: "boolean",
        ...
        section: {
          type: "string",
          description:
            'Section to edit: heading (Markdown), top-level key (YAML/JSON), or "rows:1-50" (CSV). Returns raw section content for Edit old_string.',
        },
      },
      required: ["path"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        lang: {
          type: "string",
          description:
            'Filter by language/extension (e.g., "php", "typescript")',
        },
        context_lines: {
          type: "number",
          description:
            "Lines of source context around each match (0-10). When set, shows surrounding code — saves follow-up read_symbol calls.",
        },
        mode: {
          type: "string",
          enum: ["full", "list"],
          description:
            "Output mode: full (with context, default), list (file:line only, 5-10x smaller for initial discovery)",
        },
      },
      required: ["symbol"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        include: {
          ...
          description:
            'Sections to include (default: all). Use ["stack"] for quick type check, ["quality","ci"] for tooling overview.',
        },
      },
    },
  },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        max_depth: {
          type: "number",
          description:
            "Max recursion depth when recursive=true (default: 2, max: 5)",
        },
      },
      required: ["path"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        verbose: {
          type: "boolean",
          description:
            "Show detailed breakdown: per-intent, savings by category, decision insights (default: false)",
        },
      },
    },
  },
    ...
    inputSchema: {
      ...
      properties: {
        check: {
          ...
          description:
            'What to check: "pattern" (structural search via ast-grep, e.g. "except:", "print($$$ARGS)"), "todo" (TODO/FIXME comments), "deprecated" (deprecated symbols), "annotations" (find by decorator name), "all" (todo + deprecated summary)',
        },
        pattern: {
          type: "string",
          description:
            'Code pattern for check="pattern". ast-grep syntax: "except:" finds bare excepts, "print($$$ARGS)" finds print calls.',
        },
        name: {
          type: "string",
          description:
            'Decorator/annotation name for check="annotations". Example: "Deprecated", "Controller"',
        },
        lang: {
          type: "string",
          description:
            'Language filter for check="pattern" (e.g., "python", "typescript")',
        },
    ...
    inputSchema: {
      ...
      properties: {
        module: {
          type: "string",
          description:
            'Module name or path pattern (e.g., "auth", "src/Domain/")',
        },
        check: {
          type: "string",
          enum: ["deps", "dependents", "api", "unused-deps", "all"],
          description:
            'What to check: "deps" (dependencies), "dependents" (who depends on this), "api" (public symbols), "unused-deps" (dead dependencies), "all" (everything). Default: "all"',
        },
      },
      required: ["module"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        ...
        ref: {
          type: "string",
          description:
            'Git ref — required for scope="commit" (commit hash) or scope="branch" (branch name)',
        },
      },
    },
  },
    ...
    inputSchema: {
      ...
      properties: {
        path: {
          type: "string",
          description:
            "Directory path (or file path — will use its parent directory)",
        },
        include: {
          type: "array",
          items: {
    ...
    inputSchema: {
      ...
      properties: {
        command: {
          type: "string",
          description:
            'Test command to run (e.g., "npm test", "pytest", "go test ./...")',
        },
        runner: {
          type: "string",
          enum: [
    ...
    inputSchema: {
      ...
      properties: {
        ...
        decisions: {
          ...
          items: { type: "string" },
          description:
            'Key decisions made and why (e.g., "removed sysfee step — caused double counting"). Prevents revisiting rejected approaches.',
        },
        confirmed: {
          type: "array",
          items: { type: "string" },
    ...
    inputSchema: {
      ...
      properties: {
        sessionId: {
          type: "string",
          description:
            'Claude Code session_id (same id that appears in hook-events.jsonl). Pass "" to read with no session filter.',
        },
      },
      required: ["sessionId"],
    },
