...
export function validateReadSymbolsArgs(args: unknown): {
  ...
  if (!Array.isArray(a.symbols) || a.symbols.length === 0) {
    throw new Error(
      'Required parameter "symbols" must be a non-empty array of strings.',
    );
  }
 ...
 */
export function validateSmartReadManyArgs(args: unknown): {
  paths: string[];
  max_tokens?: number;
} {
  if (!args || typeof args !== "object") {
...
export function validateReadForEditArgs(args: unknown): {
  ...
  if (
    !a.symbol &&
    !a.line &&
    (!Array.isArray(a.symbols) || (a.symbols as unknown[]).length === 0) &&
    !a.section
  ) {
    throw new Error(
      'Either "symbol", "symbols", "line", or "section" must be provided.',
    );
  }
...
export function validateCallTreeArgs(args: unknown): {
  symbol: string;
  depth?: number;
} {
  if (!args || typeof args !== "object") {
  ...
  if (!symbol) {
    throw new Error(
      "call_tree: `symbol` is required and must be a non-empty string.",
    );
  }
  return {
    symbol,
...
export function validateCodeAuditArgs(args: unknown): CodeAuditArgs {
  ...
  if (typeof a.check !== "string" || !validChecks.includes(a.check)) {
    throw new Error(
      `Required parameter "check" must be one of: ${validChecks.join(", ")}`,
    );
  }
  ...
  if (a.check === "pattern") {
    if (typeof a.pattern !== "string" || a.pattern.length === 0) {
      throw new Error(
        'Parameter "pattern" is required when check="pattern". Example: "except:" or "print($$$ARGS)"',
      );
    }
  }
  ...
  if (a.check === "annotations") {
    if (typeof a.name !== "string" || a.name.length === 0) {
      throw new Error(
        'Parameter "name" is required when check="annotations". Example: "Deprecated" or "Controller"',
      );
    }
  }
...
export function validateProjectOverviewArgs(
  args: unknown,
): ProjectOverviewArgs {
  if (!args || typeof args !== "object") return {};
  const a = args as Record<string, unknown>;
  ...
  if (a.include !== undefined && a.include !== null) {
    ...
    for (const item of a.include) {
      if (
        typeof item !== "string" ||
        !(VALID_INCLUDE_SECTIONS as readonly string[]).includes(item)
      ) {
        throw new Error(
          `Each element of "include" must be one of: ${VALID_INCLUDE_SECTIONS.join(", ")}. Got: "${item}"`,
        );
      }
    }
...
export function validateExploreAreaArgs(args: unknown): ExploreAreaArgs {
  ...
  if (a.include !== undefined && a.include !== null) {
    ...
    for (const item of a.include) {
      if (
        typeof item !== "string" ||
        !(VALID_EXPLORE_SECTIONS as readonly string[]).includes(item)
      ) {
        throw new Error(
          `Each element of "include" must be one of: ${VALID_EXPLORE_SECTIONS.join(", ")}. Got: "${item}"`,
        );
      }
    }
...
export function validateTestSummaryArgs(args: unknown): TestSummaryArgs {
  ...
  if (timeout !== undefined) {
    if (!Number.isInteger(timeout) || timeout < 1000 || timeout > 300000) {
      throw new Error(
        '"timeout" must be an integer between 1000 and 300000 (ms).',
      );
    }
  }
...
export function validateReadSectionArgs(args: unknown): {
  path: string;
  heading: string;
} {
  if (!args || typeof args !== "object") {
