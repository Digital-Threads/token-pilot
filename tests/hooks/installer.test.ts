...
describe("Hook Installer", () => {
  ...
  it("does NOT emit an args field (v0.34.1 — Claude Code skips env expansion in args, ENOENT regression)", async () => {
    // v0.34.0 briefly added `args: string[]` alongside `command` to
    // adopt the new direct-spawn schema. Real installs hit ENOENT
    // because Claude Code did not expand `${CLAUDE_PLUGIN_ROOT}`
    // inside `args` array elements. v0.34.1 reverted. This guard
    // makes sure a future PR that re-adds `args` without first
    // proving env-expansion works in production fails CI loudly.
    const result = await installHook(tempDir, {
