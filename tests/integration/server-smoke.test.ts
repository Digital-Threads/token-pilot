...
vi.mock('../../src/ast-index/client.js', () => ({
  AstIndexClient: class {
    ...
    isOversized(): boolean { return false; }
    ...
    async outline(filePath: string): Promise<any> {
      ...
      return {
        path: filePath,
        language: 'TypeScript',
        meta: { lines: 240, bytes: 10_000, lastModified: Date.now(), contentHash: 'hash' },
        imports: [],
        exports: [],
        symbols: [{
          name: 'app',
    ...
    async refs(symbol: string): Promise<any> {
      ...
      return {
        definitions: [{ path: join(mockState.root, 'app.ts'), line: 1, name: 'app', signature: 'export function app()' }],
        imports: [{ path: join(mockState.root, 'consumer.ts'), line: 1, name: 'app', context: 'import { app } from "./app"' }],
        usages: [{ path: join(mockState.root, 'consumer.ts'), line: 3, name: 'app', context: 'app();' }],
      };
    }
    async search(): Promise<any[]> {
      return [];
    }
    async map(): Promise<any> { return null; }
    async conventions(): Promise<any> { return null; }
    async stats(): Promise<string | null> { return null; }
    async fileImports(): Promise<any[]> { return []; }
...
describe('createServer smoke test', () => {
  ...
  beforeEach(async () => {
    ...
    const appContent = Array.from({ length: 240 }, (_, i) => `export const line${i} = ${i};`).join('\n');
    await writeFile(join(tempDir, 'app.ts'), appContent);
    await writeFile(join(tempDir, 'consumer.ts'), 'import { app } from "./app";\napp();\n');
    await writeFile(join(tempDir, 'package.json'), JSON.stringify({ name: 'smoke-app', version: '1.0.0' }));
  });
  ...
  it('starts a real MCP server, lists tools, and serves tool calls over a transport', async () => {
    ...
    const readResult = await client.callTool({ name: 'smart_read', arguments: { path: 'app.ts' } });
    expect(readResult.content?.[0]?.type).toBe('text');
    expect(readResult.content?.[0]?.text).toContain('TOKEN SAVINGS');

    const usagesResult = await client.callTool({ name: 'find_usages', arguments: { symbol: 'app' } });
