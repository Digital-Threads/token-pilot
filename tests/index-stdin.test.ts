...
async function loadIndexWithReadFile(mockReadFileSync: ReturnType<typeof vi.fn>) {
  vi.resetModules();
...
describe('index stdin hooks', () => {
  ...
  it('allows bounded hook-read requests from stdin and exits quietly on invalid input', async () => {
    ...
    expect(() => boundedModule.handleHookRead()).toThrow('EXIT:0');
    expect(writeSpy).not.toHaveBeenCalled();

    const invalidRead = vi.fn(() => '{bad json');
    const invalidModule = await loadIndexWithReadFile(invalidRead);
    expect(() => invalidModule.handleHookRead()).toThrow('EXIT:0');
    expect(writeSpy).not.toHaveBeenCalled();
  });
  ...
  it('denies large unbounded code reads parsed from stdin', async () => {
    ...
    expect(() => mod.handleHookRead()).toThrow('EXIT:0');
  ...
  it('adds edit guidance for code files and skips non-code or malformed input', async () => {
    const codeEditRead = vi.fn(() => JSON.stringify({
      tool_input: {
    ...
    expect(writeSpy).toHaveBeenCalledTimes(1);
    expect(String(writeSpy.mock.calls[0][0])).toContain('"permissionDecision":"allow"');
    expect(String(writeSpy.mock.calls[0][0])).toContain('read_for_edit(\\"/repo/file.ts\\"');

    writeSpy.mockClear();
    const docEditRead = vi.fn(() => JSON.stringify({
      tool_input: {
  ...
  it('auto-runs main when imported directly and reports fatal startup failures', async () => {
    ...
    try {
      ...
      mockDeps.installHook.mockResolvedValue({ installed: false, fatal: false, message: 'skipped' });
      ...
      await loadIndexFresh();
      await new Promise((resolve) => setTimeout(resolve, 0));
      expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('Fatal: startup failed'))).toBe(true);
