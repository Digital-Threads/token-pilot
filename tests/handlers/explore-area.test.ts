...
describe("handleExploreArea", () => {
  ...
  beforeEach(async () => {
    ...
    await writeFile(
      join(tempDir, "shared", "util.ts"),
      "export function util() {}\n",
    );
    await writeFile(
      join(tempDir, "tests", "feature.test.ts"),
    ...
    await execFileAsync("git", ["config", "user.name", "Token Pilot Tests"], {
      cwd: tempDir,
    });
  ...
  // v0.30.0 — narrowed default from ["outline","imports","tests","changes"]
  // (all 4) to ["outline","changes"]. Tool-audit telemetry in docker-local-env
  // showed the all-4 default at -7% token reduction on 7 calls — returning
  // more tokens than a raw read baseline. Imports/tests are now opt-in.
  it("defaults to outline+changes only; imports and tests are opt-in", async () => {
    const astIndex = {
      isAvailable: () => true,
      isDisabled: () => false,
      isOversized: () => false,
    ...
    const text = result.content[0].text;
    // Outline + changes present
    expect(text).toContain("STRUCTURE:");
    expect(text).toContain("RECENT CHANGES:");
    // Imports + tests sections NOT rendered — meta arrays must be empty
    expect(text).not.toContain("IMPORTS:");
    expect(text).not.toContain("TESTS:");
    expect(result.meta.internalDeps).toEqual([]);
    expect(result.meta.importedBy).toEqual([]);
    expect(result.meta.externalDeps).toEqual([]);
    expect(result.meta.testFiles).toEqual([]);
    // Outline + changes still populated
    expect(result.meta.codeFiles).toEqual(["src/feature.ts"]);
    expect(result.meta.changeCount).toBeGreaterThan(0);
  });
});
