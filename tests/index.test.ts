...
vi.mock('../src/core/validation.js', async () => {
  const actual = await vi.importActual<typeof import('../src/core/validation.js')>('../src/core/validation.js');
  return {
    ...actual,
...
describe('index CLI helpers', () => {
  ...
  beforeEach(async () => {
    ...
    mockDeps.installHook.mockResolvedValue({ installed: false, fatal: false, message: 'hook skipped' });
    mockDeps.uninstallHook.mockResolvedValue({ removed: false, fatal: false, message: 'hook removed' });
    mockDeps.checkBinaryUpdate.mockResolvedValue({ updateAvailable: false, current: '1.0.0', latest: '1.0.0' });
    mockDeps.isNewerVersion.mockReturnValue(false);
    mockDeps.detectContextMode.mockResolvedValue({ detected: false, source: 'none', toolPrefix: '' });
  });
  ...
  it('dispatches version and help through main', async () => {
    ...
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('MCP Tools (18)'))).toBe(true);
  });
  ...
  it('dispatches the default main path to server startup', async () => {
    ...
    expect(mockDeps.createServer).toHaveBeenCalledWith(tempDir, { skipAstIndex: false });
  });
  ...
  it('denies unbounded reads of large code files and allows small/non-code files', async () => {
    ...
    await writeFile(largeFile, Array.from({ length: 600 }, (_, i) => `line ${i}`).join('\n'));
    await writeFile(smallFile, 'const x = 1;\n');
    await writeFile(mdFile, '# docs\n');

    expect(() => indexModule.handleHookRead(mdFile)).toThrow('EXIT:0');
    expect(writeSpy).not.toHaveBeenCalled();

    expect(() => indexModule.handleHookRead(smallFile)).toThrow('EXIT:0');
    expect(writeSpy).not.toHaveBeenCalled();

    expect(() => indexModule.handleHookRead(largeFile)).toThrow('EXIT:0');
  ...
  it('installs and uninstalls hooks with the returned fatal flag', async () => {
    mockDeps.installHook.mockResolvedValueOnce({ installed: true, fatal: false, message: 'installed' });
    await expect(indexModule.handleInstallHook(tempDir)).rejects.toThrow('EXIT:0');
    expect(logSpy).toHaveBeenCalledWith('installed');

    mockDeps.uninstallHook.mockResolvedValueOnce({ removed: false, fatal: true, message: 'broken' });
    await expect(indexModule.handleUninstallHook(tempDir)).rejects.toThrow('EXIT:1');
  ...
  it('installs ast-index, reports up-to-date state, and handles failures', async () => {
    ...
    mockDeps.installBinary.mockResolvedValueOnce({ path: '/bin/ast-index', version: '1.1.0' });
    await indexModule.handleInstallAstIndex();
    expect(exitSpy).toHaveBeenLastCalledWith(0);
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('installed to /bin/ast-index'))).toBe(true);
    ...
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('network failed'))).toBe(true);
  });

  it('creates and updates .mcp.json via init', async () => {
    await expect(indexModule.handleInit(tempDir)).rejects.toThrow('EXIT:0');
    const created = JSON.parse(await readFile(join(tempDir, '.mcp.json'), 'utf-8'));
    expect(created.mcpServers['token-pilot']).toBeTruthy();
    expect(created.mcpServers['context-mode']).toBeTruthy();

    await expect(indexModule.handleInit(tempDir)).rejects.toThrow('EXIT:0');
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('already has both'))).toBe(true);

    const partialDir = await mkdtemp(join(tmpdir(), 'token-pilot-index-partial-'));
    await writeFile(join(partialDir, '.mcp.json'), JSON.stringify({
      mcpServers: {
        'token-pilot': { command: 'npx', args: ['-y', 'token-pilot'] },
      },
    }));
    await expect(indexModule.handleInit(partialDir)).rejects.toThrow('EXIT:0');
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes(`Updated ${join(partialDir, '.mcp.json')} — added: context-mode`))).toBe(true);
    await rm(partialDir, { recursive: true, force: true });

    const brokenDir = await mkdtemp(join(tmpdir(), 'token-pilot-index-broken-'));
  ...
  it('checks npm latest safely and reports updates on startup', async () => {
    ...
    await expect(indexModule.checkNpmLatest('token-pilot')).resolves.toBe('9.9.9');
    ...
    mockDeps.installBinary.mockResolvedValue({ path: '/bin/ast-index', version: '1.1.0' });
    ...
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('Auto-updating ast-index'))).toBe(true);
  ...
  it('starts the server for explicit roots and warns on dangerous roots', async () => {
    ...
    mockDeps.createServer.mockResolvedValue(server);
    mockDeps.installHook.mockResolvedValueOnce({ installed: true, fatal: false, message: 'hook installed' });

    await indexModule.startServer([tempDir]);
    expect(mockDeps.createServer).toHaveBeenCalledWith(tempDir, { skipAstIndex: false });
    expect(server.connect).toHaveBeenCalled();
    await new Promise((resolve) => setTimeout(resolve, 0));
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('hook auto-installed: hook installed'))).toBe(true);

    await indexModule.startServer(['/']);
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('too broad'))).toBe(true);
    expect(mockDeps.createServer).toHaveBeenCalledWith('/', { skipAstIndex: true });
  });

  it('auto-detects git root when started without explicit args', async () => {
    const repoDir = await realpath(await mkdtemp(join(tmpdir(), 'token-pilot-index-git-')));
    ...
    await indexModule.startServer([]);
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes(`project root: ${repoDir}`))).toBe(true);
    expect(mockDeps.createServer).toHaveBeenCalledWith(repoDir, { skipAstIndex: false });
  ...
  it('falls back to INIT_CWD when auto-detect cannot find a git root', async () => {
    const fallbackDir = await mkdtemp(join(tmpdir(), 'token-pilot-index-fallback-'));
    const originalInitCwd = process.env.INIT_CWD;
    ...
    await indexModule.startServer([]);
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes(`${fallbackDir} (INIT_CWD, not a git repo)`))).toBe(true);
    expect(mockDeps.createServer).toHaveBeenCalledWith(fallbackDir, { skipAstIndex: false });
  ...
  it('prints doctor diagnostics for installed components', async () => {
    ...
    const fetchMock = vi.fn()
      .mockResolvedValueOnce({ ok: true, json: async () => ({ version: '9.9.9' }) })
      .mockResolvedValueOnce({ ok: true, json: async () => ({ version: '3.3.3' }) });
    vi.stubGlobal('fetch', fetchMock);
    mockDeps.isNewerVersion.mockReturnValue(true);

    await expect(indexModule.handleDoctor()).rejects.toThrow('EXIT:0');
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('token-pilot doctor'))).toBe(true);
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('latest:       9.9.9'))).toBe(true);
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('auto-update:  enabled'))).toBe(true);
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('detected:     yes (mcp-json)'))).toBe(true);
  ...
  it('prints doctor setup guidance when context-mode is missing', async () => {
    ...
    await expect(indexModule.handleDoctor()).rejects.toThrow('EXIT:0');
    expect(logSpy.mock.calls.some((call) => String(call[0]).includes('setup:        npx token-pilot init'))).toBe(true);
  ...
  it('logs non-auto ast-index updates without forcing installation', async () => {
    ...
    expect(errorSpy.mock.calls.some((call) => String(call[0]).includes('ast-index update: 1.0.0 → 1.2.0'))).toBe(true);
