...
import { checkPolicy, isFullReadTool, DEFAULT_POLICIES } from '../../src/core/policy-engine.js';
...
describe('checkPolicy', () => {
  ...
  it('advises read_for_edit before edit when not called', () => {
    const result = checkPolicy(DEFAULT_POLICIES, 'edit', {
      fullFileReadsCount: 0,
      tokensReturned: 0,
      readForEditCalled: new Set(),
      editTargetPath: 'src/foo.ts',
    });

    expect(result).not.toBeNull();
    expect(result!.message).toContain('read_for_edit');
  });

  it('does not advise when read_for_edit was called', () => {
    const result = checkPolicy(DEFAULT_POLICIES, 'edit', {
      fullFileReadsCount: 0,
      tokensReturned: 0,
      readForEditCalled: new Set(['src/foo.ts']),
      editTargetPath: 'src/foo.ts',
    });
  ...
  it('respects disabled policies', () => {
    const disabledPolicies = {
      ...
      maxFullFileReads: 0,
      requireReadForEditBeforeEdit: false,
    };
